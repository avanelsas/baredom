...
6. Commit on a branch and merge it via PR, per the rule above
7. Pull `main`, then tag the merge commit and push the tag:
...
**Tag after the merge, not before.** The tag must point at the squash-merge commit on `main`; the branch commit it was built from is discarded by the squash and tagging it would publish a commit that is on no branch. `v3.6.0`, `v3.7.0` and `v3.7.1` all point at their merge commits.
