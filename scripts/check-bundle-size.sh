...
# detail pane, sessions strip, and import/export UI, so 15 KB is the
# wrong ceiling for it. The dock is opt-in via ?baredom-trace-history
# and never runs in production builds where the flag stays off.
...
module_budget() {
  case "$1" in
    ...
    x-trace-history.js)  echo 20480 ;;  # 20 KB for the dev-tool dock
