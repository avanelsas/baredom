...
(defn- init-dom! [^js el]
  (let [root       (.attachShadow el #js {:mode "open"})
    ...
    (du/set-attr! img      attr-alt  "")
    (du/set-attr! initials attr-part part-initials)
    (du/set-attr! fallback attr-part part-fallback)
    (set! (.-textContent fallback) val-fallback)
    (du/set-attr! status   attr-part        part-status)
    (du/set-attr! status   attr-aria-hidden val-true)
    (du/set-attr! badge      attr-part part-badge)
    (du/set-attr! badge-slot attr-name slot-name-badge)
