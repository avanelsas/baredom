...
(def ^:private style-text
  (str
   ...
   "flex-wrap:wrap;"
   "}"
   ":host(:not([orientation=vertical])) [part='nav']{order:7;}"
   ":host(:not([orientation=vertical])) [part='actions']{order:8;justify-self:start;}"
   ":host(:not([orientation=vertical])) [part='end']{order:9;}"
   "}"
