...
(def ^:private style-text
  (str
   ...
   "}"
   ":host(:not([orientation=vertical])) [part='brand']{order:1;}"
   ":host(:not([orientation=vertical])) [part='start']{order:2;justify-self:start;}"
   ":host(:not([orientation=vertical])) [part='nav']{"
   "order:3;grid-column:1/-1;flex-wrap:wrap;}"
   ":host(:not([orientation=vertical])) [part='actions']{"
   "order:4;grid-column:1/-1;flex-wrap:wrap;justify-self:start;}"
   ":host(:not([orientation=vertical])) [part='toggle']{"
   "order:5;}"
   ":host(:not([orientation=vertical])) [part='end']{"
   "order:6;justify-self:end;}"
   "}"
