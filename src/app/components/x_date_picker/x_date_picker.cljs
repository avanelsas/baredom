(ns app.components.x-date-picker.x-date-picker
  (:require [goog.object :as gobj]
            [app.components.x-date-picker.model :as model]))
  ...
  (let [state (gobj/get el k-state)
    ...
    (doseq [{:keys [date in-month?]} items]
      (let [^js btn  (make-el "button")
        ...
        (when disabled? (.setAttribute btn "disabled" ""))
...
(defn- do-select-date!
  ...
  (let [state  (gobj/get el k-state)
        ...
        detail #js {:iso (model/date->iso d) :reason reason}
        ^js ev (dispatch! el model/event-change-request true detail)]
    (when-not (.-defaultPrevented ev)
      (if (= mode :single)
        (do
          ...
          (read-state! el)
          (render! el)
          (dispatch! el model/event-change false detail)
...
(defn- commit-display!
  ...
  (let [refs    (gobj/get el k-refs)
        ...
        canon   (when state (gobj/get state "canon"))
        mode    (:mode canon)]
    (dispatch! el model/event-input false #js {:value val})
    (if (= mode :single)
      (let [{:keys [ok? date]} (model/parse-display->single val)]
        (when ok?
          (let [^js ev (dispatch! el model/event-change-request true
                                  #js {:iso (model/date->iso date) :reason reason})]
            (when-not (.-defaultPrevented ev)
      ...
      (let [{:keys [ok? start end]} (model/parse-display->range val {:separator (:separator canon)})]
        (when ok?
          (let [^js ev (dispatch! el model/event-change-request true
                                  #js {:start-iso (model/date->iso start)
                                       :end-iso   (model/date->iso end)
                                       :reason    reason})]
            (when-not (.-defaultPrevented ev)
              (.setAttribute el model/attr-start (model/date->iso start))
              (.setAttribute el model/attr-end   (model/date->iso end))
              (gobj/set el k-range-step 0)
              (read-state! el)
              (render! el)
              (dispatch! el model/event-change false
...
(def ^:private style-text
  (str
   ...
   ":host{"
   "--x-date-picker-border:#334155;"
   "--x-date-picker-input-bg:#0f172a;"
   "--x-date-picker-btn-bg:#1e293b;"
   "--x-date-picker-btn-hover:#334155;"
   "--x-date-picker-popover-bg:#1e293b;"
   "--x-date-picker-day-hover:#334155;"
   "--x-date-picker-selected-bg:#3b82f6;"
   "--x-date-picker-range-bg:#1e3a8a;"
   "--x-date-picker-range-text:#93c5fd;"
   "--x-date-picker-nav-hover:#334155;"
   "}"
   "}"
...
(defn- on-input-event!
  ...
  (let [refs    (gobj/get el k-refs)
    ...
    (dispatch! el model/event-input false #js {:value (when inp (.-value inp))})))
...
(defn- element-class
  ...
  (let [ctor-ref (atom nil)
        ...
        _        (reset! ctor-ref ctor)
        proto    (js/Object.create (.-prototype js/HTMLElement))]
