(ns app.components.x-table.x-table
  (:require
   [goog.object :as gobj]
...
(defn- handle-single-select! [^js el ^js target-row]
  ;; Deselect all currently-selected rows, then select the target.
  (let [^js selected-rows (.querySelectorAll el "x-table-row[selected]")]
  ...
  (let [m          (or (gobj/get el k-model) (read-model el))
    ...
    (when (not= selectable "none")
      (let [^js row      (.-target e)
            ...
            ;; The row will be selected after we act; determine its post-action state
            currently?   (.hasAttribute row "selected")
            will-select? (if (= selectable "single")
                           ;; In single mode, clicking always selects (deselects if already selected via toggle)
                           (not currently?)
                           (not currently?))]
        (dispatch-row-select! el row-index will-select? selectable)
        ;; Note: we fire the event unconditionally and handle DOM after.
        ;; The app can call preventDefault on x-table-row-click before it
        ;; reaches x-table, or listen to x-table-row-select.
        (cond
