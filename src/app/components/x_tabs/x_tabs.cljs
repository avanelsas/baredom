(ns app.components.x-tabs.x-tabs
  (:require
   [goog.object :as gobj]
...
(defn set-host-value-if-needed! [el value]
...
(defn set-tab-selected-if-needed! [tab selected?]
...
(defn set-tab-tabindex-if-needed! [tab tabindex]
...
(defn dispatch-value-change! [el value]
  (.dispatchEvent
   el
   (js/CustomEvent.
...
(defn activate-tab-by-value! [el value]
  (let [current (.getAttribute el model/attr-value)]
...
(defn install-mutation-observer! [el]
...
(defn disconnect-mutation-observer! [el]
...
(defn disconnected-callback [el]
  (remove-listeners! el))

(defn attribute-changed-callback [el _name _old _new]
  (when (initialized? el)
    (render! el)))
