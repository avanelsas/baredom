(ns app.components.x-card.x-card
  (:require
   [goog.object :as gobj]
...
(defn get-instance-value [el key]
  (gobj/get el key))

(defn set-instance-value! [el key value]
...
(defn shadow-root-of [el]
  (get-instance-value el key-root))

(defn base-node-of [el]
...
(defn set-or-remove-attr! [el name value]
  (if (some? value)
    (.setAttribute el name value)
    (.removeAttribute el name)))

(defn reflect-host-a11y! [el state]
...
(defn reflect-base-state! [base state]
...
(defn on-click [el _event]
  (let [state (model/derive-state (read-inputs el))]
    (when (interactive-active? state)
      (dispatch-press! el))))

(defn on-keydown [el event]
  (let [state (model/derive-state (read-inputs el))
        key (.-key event)]
    (when (interactive-active? state)
      (cond
...
(defn disconnected-callback [_el])

(defn attribute-changed-callback [el _name _old-value _new-value]
  (when (initialized? el)
    (render! el)))
