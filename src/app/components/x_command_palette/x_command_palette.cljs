(ns app.components.x-command-palette.x-command-palette
  (:require [goog.object :as gobj]
            [app.components.x-command-palette.model :as model]))
...
(def ^:private style-text
  (str
   ...
   "border-radius:var(--x-command-palette-radius,12px);"
   "background:var(--x-command-palette-bg,#fff);"
   ...
   "color:var(--x-command-palette-icon-color,#64748b);"
   "}"
   "[part=input]{"
   ...
   "color:var(--x-command-palette-placeholder,#94a3b8);"
   "}"
   "[part=clear-btn]{"
   ...
   "color:var(--x-command-palette-icon-color,#64748b);"
   "}"
   "[part=clear-btn]:focus-visible{outline:2px solid var(--x-command-palette-focus-ring,#60a5fa);}"
   ...
   "background:var(--x-command-palette-item-hover,#f1f5f9);"
   "}"
   ...
   "color:var(--x-command-palette-group-text,#94a3b8);"
   "}"
   "[part=empty]{"
   "padding:32px 16px;text-align:center;"
   "color:var(--x-command-palette-empty-text,#94a3b8);"
   "font-size:0.9375rem;"
   "}"
   "[part=empty][hidden]{display:none;}"
   "@media (prefers-color-scheme:dark){"
   "[part=panel]{background:var(--x-command-palette-bg,#1e293b);box-shadow:0 20px 60px rgba(0,0,0,0.6),0 4px 16px rgba(0,0,0,0.4);}"
   "[part=search-wrap]{border-bottom-color:var(--x-command-palette-divider,rgba(255,255,255,0.08));}"
   "[part=item]:hover,[part=item]:focus{background:var(--x-command-palette-item-hover,#334155);}"
...
(defn- define-methods!
  ...
  (aset proto "close"
        (fn []
          (this-as ^js this (request-close! this))))
  (aset proto "toggle"
        (fn []
          (this-as ^js this
