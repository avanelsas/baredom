(ns app.components.x-notification-center.x-notification-center
  (:require
   [goog.object :as gobj]
   [app.components.x-notification-center.model :as model]
   [app.components.x-alert.model :as alert-model]))
...
(defn- on-alert-dismiss [^js el ^js e]
  (let [refs      (ensure-refs! el)
        ...
        path      (.composedPath e)
        ^js alert (aget path 0)
        id        (.getAttribute alert model/data-notification-id)
        ;; querySelectorAll returns live result; dismiss already happened so count is now minus 1
