(ns app.components.x-cancel-dialogue.model)
...
(def observed-attributes
  #js ["open" "disabled" "headline" "message" "confirm-text" "cancel-text" "danger" "portal"])
...
(defn normalize
  ...
  [{:keys [open-present? disabled-present? headline-raw message-raw
           confirm-text-raw cancel-text-raw danger-present? portal-raw]}]
  {:open?        (boolean open-present?)
   ...
   :portal       (when (and (string? portal-raw) (not= (.trim portal-raw) ""))
                   (.trim portal-raw))})
