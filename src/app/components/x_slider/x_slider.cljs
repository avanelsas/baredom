(ns app.components.x-slider.x-slider
...
(defn- make-keydown-handler [^js el]
  (fn [^js evt]
    (when (or (has-attr? el model/attr-disabled)
              (has-attr? el model/attr-readonly))
      (.preventDefault evt)
      (.stopPropagation evt))))
