(ns app.components.x-radio.x-radio
  (:require [goog.object :as gobj]
            [app.components.x-radio.model :as model]))
...
(def ^:private style-text
  (str
   ...
   "--x-radio-border-color:#6b7280;"
   "--x-radio-bg:#ffffff;"
   "--x-radio-checked-color:#2563eb;"
   "--x-radio-dot-size:6px;"
   "--x-radio-focus-ring:#60a5fa;"
   "--x-radio-disabled-opacity:0.45;"
   "--x-radio-transition:background 120ms ease,border-color 120ms ease;"
   "}"
   "@media (prefers-color-scheme:dark){"
   ":host{"
   "--x-radio-border-color:#9ca3af;"
   "--x-radio-bg:#1f2937;"
   "--x-radio-checked-color:#3b82f6;"
   "--x-radio-focus-ring:#93c5fd;"
   "}"
   "}"
   "[part=control]{"
   "all:unset;"
