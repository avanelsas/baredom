...
(defn- install-apply-resource! [^js proto render on-failure on-pending]
...
(defn register!
  ...
  [{:keys [tag child-tag render on-failure on-pending on-connect observed-attributes]}]
  (component/register!
   tag
   {:observed-attributes  (or observed-attributes #js [])
    ...
    :setup-prototype-fn   (fn [^js proto] (install-apply-resource! proto render on-failure on-pending))}))
