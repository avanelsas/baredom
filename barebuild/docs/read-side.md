...
;; Read the delivered value from e.detail.state — the event is a value and carries
;; its own payload. (event.target is for identity only — which broker — not value.)
...
3. **`.state` is a value.** `(.-state el)` returns a ClojureScript persistent map — `{:phase :data :error :http-status}` — that you destructure directly. No `js->clj`, no JS-shaped objects on the read path. Between phase transitions the same map is returned by reference (`identical?` holds), so there is no per-read allocation.
4. **Components absorb deltas via property setters.** `(set! (.-items table) data)` triggers `<x-table>`'s own render-pipeline; its model-change guard applies the delta. The element instance is never destroyed, so selection / scroll / focus survive.
...
`(set! (.-items table) rows)` triggers the table's render-pipeline. The model-change guard compares old and new, applies the delta, stops. Component-internal state (selection, hover, sort order) is preserved. There is **no top-level Hiccup re-render** in V1 — Hiccup itself is deferred.
...
(.addEventListener users-route "barebuild-data-state"
  (fn [^js e]
    (let [{:keys [phase data]} (.. e -detail -state)]
      (when (= :loaded phase) (set! (.-items users-table) data)))))
```
