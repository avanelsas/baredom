...
`x-alert`), `on-pending!` (`aria-busy` + dim), and sort/page gestures via `submit-intent!`.

Both are driven by the same 49-line `consumer_resource.cljs` — the difference between them is
exactly their projection and their hooks.
