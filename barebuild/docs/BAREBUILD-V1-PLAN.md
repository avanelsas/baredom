...
| **Values, not places** | Route configuration is markup; the match result is a value. Fetched state is *one* value (`.state` is the persistent CLJS map). **The router owns no authoritative state** — it caches projections of two external truths (the URL in `history`/`location`; the route set in the DOM), holds its route-handle registry as a succession of immutable values (not in-place mutation), and computes matches purely. Nothing owns mutable global state. |
| **Web as info system** | Navigation = `popstate` + `barebuild-navigate`. Data state = `barebuild-data-state {:state}`. Every event detail is a map; **listeners read the delivered value from `event.detail`**, and disambiguate siblings via `event.target` (identity only — which element dispatched), never by reading a place-coupled identifier from the detail. **Payloads are schema-open maps; in V1 the runtime type at leaf positions is the CLJS persistent map.** A JS-view accessor is deferred with `.stateJs`. The `.state` keys are `:phase :data :error :http-status` (no `?` suffix); `:data`/`:error`/`:http-status` are present only when meaningful for the current phase (absent, not nil-valued, otherwise). |
...
;; Read the delivered value from e.detail.state; e.target is for identity only.
...
- [ ] `.state` returns the persistent CLJS map directly with keys `:phase :data :error :http-status` (`?`-suffixed keys are not literal; `:data`/`:error`/`:http-status` are present only when meaningful for the phase). The cached map is returned by reference: assert with **`(identical? (.-state el) (.-state el))`** between phase transitions — `=` alone would pass even if a fresh equal map were rebuilt per read, so it does not prove the "no `clj->js` allocation per read" property; `identical?` does. **No `.stateJs`** — grep-asserted: zero occurrences of `stateJs` in `barebuild_data.cljs`.
...
- [ ] **`docs/barebuild-data.md` notes the JS-consumer boundary:** *"V1 `.state` returns a ClojureScript persistent map; calling code must be CLJS. Pure-JS consumers importing the ESM bundle directly should defer adoption until `.stateJs` ships (deferred decision #21)."* Prevents the first JS-from-ESM user from filing a confused "the keys look wrong" issue.
- [ ] `clj-kondo --lint src test` clean; `npx shadow-cljs release lib` passes; `npm test` green.
...
8. One `.state` property per fetcher, returning the persistent CLJS map directly. **No `.stateJs`** in V1 — CLJS audience is committed-to. Schema is open (keys `{:phase :data :error :http-status}` — `:data`/`:error`/`:http-status` present only when meaningful for the phase — and extensible); runtime type at leaf positions is the CLJS persistent map. "Open" here means schema-open, not language-portable byte-for-byte; the trade is named, not implicit.
...
21. **`.stateJs` JS-view accessor** (and event-detail JS-view). Shape sketch in `barebuild/docs/write-side-sketch.md`.
