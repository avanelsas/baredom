...
(deftest collapsed-docked-data-attribute-test
  (testing "collapsed attribute applies only in docked mode"
    (let [el (make-element!)]
      ;; Set breakpoint to 0 so viewport never overrides declared variant
      (.setAttribute el model/attr-breakpoint "0")
      (.setAttribute el model/attr-variant model/variant-docked)
      (.setAttribute el model/attr-collapsed "")
      (is (= "true" (.getAttribute el "data-collapsed")))
      ;; collapsed is ignored in overlay mode
      (.setAttribute el model/attr-variant model/variant-overlay)
      (is (= "false" (.getAttribute el "data-collapsed"))))))

(deftest right-placement-data-attribute-test
  (testing "right placement is projected to host"
    (let [el (make-element!)]
      (.setAttribute el model/attr-placement model/placement-right)
      (is (= "right" (.getAttribute el "data-placement"))))))

(deftest backdrop-inert-attribute-test
  (testing "backdrop inert attribute reflects show-backdrop state"
    (let [el (make-element!)
          root (.-shadowRoot el)
          backdrop (.querySelector root ".backdrop")]
      (.setAttribute el model/attr-variant model/variant-overlay)
      ;; closed — backdrop should be inert
      (is (true? (.-hidden backdrop)))
      ;; open — backdrop should not be inert
      (set! (.-open el) true)
      (is (false? (.-hidden backdrop)))
      (when (exists? (.-inert backdrop))
        (is (false? (.-inert backdrop)))))))

(deftest programmatic-close-does-not-fire-dismiss-test
  (testing "dismiss does NOT fire on programmatic close"
    (let [el (make-element!)
          dismiss-fired (atom false)]
      (.setAttribute el model/attr-variant model/variant-overlay)
      (set! (.-open el) true)
      (.addEventListener el model/event-dismiss
                         (fn [_] (reset! dismiss-fired true)))
      ;; programmatic close via property
      (set! (.-open el) false)
      (is (false? @dismiss-fired)))))
