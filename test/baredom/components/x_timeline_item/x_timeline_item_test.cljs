(ns baredom.components.x-timeline-item.x-timeline-item-test
  (:require [cljs.test :refer-macros [deftest is use-fixtures]]
            [baredom.components.x-timeline-item.x-timeline-item :as x]
...
;; ── icon attribute ───────────────────────────────────────────────────────────
(deftest custom-icon-overrides-default-test
  (let [^js el (append! (make-el))]
    (.setAttribute el model/attr-icon "★")
    (is (= "★" (.-textContent (shadow-child el ".default-icon"))))))

(deftest icon-none-hides-marker-icon-test
  (let [^js el (append! (make-el))]
    (.setAttribute el model/attr-icon "none")
    (is (= "" (.-textContent (shadow-child el ".default-icon"))))))

(deftest icon-property-getter-test
  (let [^js el (append! (make-el))]
    (.setAttribute el model/attr-icon "★")
    (is (= "★" (.-icon el)))))

;; ── title property ───────────────────────────────────────────────────────────
(deftest title-property-setter-test
  (let [^js el (append! (make-el))]
    (set! (.-title el) "My Title")
    (is (= "My Title" (.getAttribute el model/attr-title)))
    (is (= "My Title" (.-textContent (shadow-child el ".title"))))))

;; ── data-striped (parent sets) ───────────────────────────────────────────────
(deftest data-striped-attribute-observed-test
  (let [^js el (append! (make-el))]
    (.setAttribute el model/data-attr-striped "")
    (is (.hasAttribute el model/data-attr-striped))))

;; ── event bubbles & composed ─────────────────────────────────────────────────
(deftest click-event-bubbles-and-composed-test
  (let [^js el   (append! (make-el))
        detail   (atom nil)]
    (.addEventListener el model/event-click
                       (fn [^js e] (reset! detail {:bubbles (.-bubbles e)
                                                   :composed (.-composed e)})))
    (.click el)
    (is (true? (:bubbles @detail)))
    (is (true? (:composed @detail)))))
