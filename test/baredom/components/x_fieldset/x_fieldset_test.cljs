(ns baredom.components.x-fieldset.x-fieldset-test
  (:require [cljs.test :refer-macros [deftest is testing use-fixtures]]
            [baredom.components.x-fieldset.x-fieldset :as x]
...
(deftest disabled-sets-inert-on-content-test
  (let [el         (append! (make-el))
        content-el (shadow-part el "[part=content]")]
    (.setAttribute el model/attr-disabled "")
    (is (.hasAttribute content-el "inert")
        "content should have inert when disabled")
    (.removeAttribute el model/attr-disabled)
    (is (not (.hasAttribute content-el "inert"))
        "content should not have inert when not disabled")))
...
(deftest legend-cleared-hides-test
  (let [el        (append! (make-el))
        legend-el (shadow-part el "[part=legend]")]
    (.setAttribute el model/attr-legend "Billing")
    (is (not (.hasAttribute legend-el "hidden")))
    (.setAttribute el model/attr-legend "")
    (is (.hasAttribute legend-el "hidden")
        "legend should hide when set back to empty")))

(deftest aria-label-overrides-labelledby-with-legend-test
  (let [el      (append! (make-el))
        root-el (shadow-part el "[part=root]")]
    (.setAttribute el model/attr-legend "Address")
    (.setAttribute el model/attr-aria-label "Custom label")
    (is (= "Custom label" (.getAttribute root-el "aria-label"))
        "aria-label should be set")
    (is (nil? (.getAttribute root-el "aria-labelledby"))
        "aria-labelledby should be removed when aria-label is present")))

(deftest aria-describedby-removed-test
  (let [el      (append! (make-el))
        root-el (shadow-part el "[part=root]")]
    (.setAttribute el model/attr-aria-describedby "desc-1")
    (is (= "desc-1" (.getAttribute root-el "aria-describedby")))
    (.removeAttribute el model/attr-aria-describedby)
    (is (nil? (.getAttribute root-el "aria-describedby"))
        "aria-describedby should be removed from root")))

(deftest disabled-property-getter-test
  (let [el (append! (make-el))]
    (is (= false (.-disabled el))
        "disabled should be false by default")
    (.setAttribute el model/attr-disabled "")
    (is (= true (.-disabled el))
        "disabled should be true when attribute is set")))
