...
(deftest name-attribute-test
  (testing "name attribute sets textarea name"
    (sut/init!)
...
(deftest placeholder-attribute-test
  (testing "placeholder attribute sets textarea placeholder"
    (sut/init!)
    (let [^js el (mount! {"placeholder" "Enter text..."})
          ^js ta (get-textarea el)]
      (is (= "Enter text..." (.-placeholder ta))))))

(deftest minlength-attribute-test
  (testing "minlength attribute sets textarea minlength"
    (sut/init!)
    (let [^js el (mount! {"minlength" "10"})
          ^js ta (get-textarea el)]
      (is (= "10" (.getAttribute ta "minlength"))))))

(deftest autocomplete-attribute-test
  (testing "autocomplete attribute sets textarea autocomplete"
    (sut/init!)
    (let [^js el (mount! {"autocomplete" "off"})
          ^js ta (get-textarea el)]
      (is (= "off" (.-autocomplete ta))))))

(deftest required-attribute-test
  (testing "required attribute sets textarea required"
    (sut/init!)
    (let [^js el (mount! {"required" ""})
          ^js ta (get-textarea el)]
      (is (true? (.-required ta))))))

(deftest readonly-attribute-test
  (testing "readonly attribute sets textarea readOnly"
    (sut/init!)
...
(deftest name-property-test
  (testing "name property reflects attribute"
    (sut/init!)
    (let [^js el (mount!)]
      (is (= "" (.-name el)))
      (set! (.-name el) "bio")
      (is (= "bio" (.-name el)))
      (is (= "bio" (.getAttribute el "name"))))))

(deftest placeholder-property-test
  (testing "placeholder property reflects attribute"
    (sut/init!)
    (let [^js el (mount!)]
      (is (= "" (.-placeholder el)))
      (set! (.-placeholder el) "Enter text")
      (is (= "Enter text" (.-placeholder el)))
      (is (= "Enter text" (.getAttribute el "placeholder"))))))

(deftest autocomplete-property-test
  (testing "autocomplete property reflects attribute"
    (sut/init!)
    (let [^js el (mount!)]
      (is (= "" (.-autocomplete el)))
      (set! (.-autocomplete el) "off")
      (is (= "off" (.-autocomplete el)))
      (is (= "off" (.getAttribute el "autocomplete"))))))

(deftest maxlength-property-test
  (testing "maxLength property reflects attribute"
    (sut/init!)
    (let [^js el (mount!)]
      (is (nil? (.-maxLength el)))
      (set! (.-maxLength el) 100)
      (is (= 100 (.-maxLength el)))
      (is (= "100" (.getAttribute el "maxlength"))))))

(deftest minlength-property-test
  (testing "minLength property reflects attribute"
    (sut/init!)
    (let [^js el (mount!)]
      (is (nil? (.-minLength el)))
      (set! (.-minLength el) 10)
      (is (= 10 (.-minLength el)))
      (is (= "10" (.getAttribute el "minlength"))))))
