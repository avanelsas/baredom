  ...
  (async done
    (let [^js el (append! (make-el))]
      ...
      (js/setTimeout
       (fn []
         (let [^js ol (shadow-part el "[part=list]")
               seps   (.querySelectorAll ol ".separator")]
           (is (= 1 (.-length seps)))
           (is (= "›" (.-textContent (aget seps 0)))))
         (done))
       0))))

;; ── Wrap attribute ────────────────────────────────────────────────────────
(deftest wrap-default-test
  (let [^js el (append! (make-el))]
    (is (= "false" (.getAttribute el "data-wrap")))))

(deftest wrap-attribute-test
  (let [^js el (append! (make-el))]
    (.setAttribute el model/attr-wrap "")
    (is (= "true" (.getAttribute el "data-wrap")))))
