(ns baredom.components.x-progress-circle.model-test
...
(deftest normalize-percent-test
  (let [s (model/normalize {:value "75" :max "100"})]
    (is (= 75    (:value s)))
    (is (= 100   (:max s)))
    (is (= 75.0  (:percent s)))
    (is (= "75%" (:aria-valuetext s)))))

(deftest normalize-indeterminate-test
  (let [s (model/normalize {:indeterminate true})]
    (is (true?  (:indeterminate s)))
    (is (= 50   (:percent s)))
    (is (= "Loading\u2026" (:aria-valuetext s)))))

(deftest normalize-label-empty-string-test
  (is (nil?          (:label (model/normalize {:label ""}))))
  (is (= "My label"  (:label (model/normalize {:label "My label"})))))

(deftest normalize-clamped-percent-test
  (let [s (model/normalize {:value "200" :max "100"})]
    (is (= 100   (:value s)))
    (is (= 100.0 (:percent s)))))
