(ns app.components.x-chart.x-chart-test
  (:require [cljs.test :refer-macros [deftest is testing use-fixtures async]]
            [app.components.x-chart.x-chart :as x]
            [app.components.x-chart.model :as model]))
...
(deftest tooltip-absent-by-default-test
  (let [el (append! (make-el))]
    ;; Without tooltip attr, tooltip part may or may not exist
    ;; (it is appended when attr is present at connect time)
    (is (some? (shadow-part el "[part=container]")))))
